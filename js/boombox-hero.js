...
camera.position.set(0, 0, isTablet() ? 10 : 11);
...
function drawWaveform() {
  ...
  for (let i = 0; i < halfBars; i++) {
    ...
    const numSegments = Math.floor(totalBarHeight / 15); // (12 + 3)
    const w = barWidth - barSpacing;

    const centerIndex = 14; 
    const xLeft = (centerIndex - i - 1) * barWidth + 1.5;
    const xRight = (centerIndex + i) * barWidth + 1.5;
...
function updateMouse(x, y, rect) {
  ...
  targetRotation.y = mouse.x * 0.349; // 20 deg
  targetRotation.x = mouse.y * 0.174; // 10 deg
}
...
loader.load(`${ASSET_BASE}/assets/models/Boombox-01.glb`, (gltf) => {
    ...
    scene.add(boombox);
    boombox.rotation.y = 0.349;
    boombox.rotation.x = -0.174;
    currentRotation.y = 0.349;
    currentRotation.x = -0.174;
...
function handleResponsiveness() {
  ...
  if (shouldRotateBoombox()) {
    boombox.rotation.z = 1.57; // 90 deg
